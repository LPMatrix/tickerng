...
## 8. Git history themes (what we learned by shipping)

Recent commits on `main` often cluster around:

- **Research agent reliability:** Vercel-compatible handler, URL resolution for dev vs prod, Tavily + OpenRouter integration, error handling.
- **Observability:** Refactors for Langfuse/tracing and clearer pipeline behavior.
- **Reports:** Schema and API updates including **soft deletion**, export/history UX.
- **Docs and env:** README, Python runtime notes, routing — treated as first-class because deployment mistakes are expensive.
- **Security hygiene:** Removing secrets from tracked `.env` (see commit history).

Use **`git log --oneline`** for the authoritative timeline when writing release notes.
