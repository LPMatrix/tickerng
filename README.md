...
src/
  ...
  lib/             # Billing, Paystack, research-agent URL; defaults + Langfuse overrides in `api/agent/`
api/
...
- Long research runs depend on platform **timeouts**. The Next route sets **`maxDuration`** in **`src/app/api/research/route.ts`**. For the **Python** handler (`/api/research-agent`), set **Maximum Duration** in the [Vercel project → Functions](https://vercel.com/docs/functions/configuring-functions/duration) dashboard (e.g. **300s**) — root **`vercel.json` `functions`** patterns often **do not match** Python entrypoints in Next.js App Router projects and can fail the build. To pin duration in-repo long term, use **[Vercel Services](https://vercel.com/docs/services)** (official Next + Python split) or rely on dashboard settings.
